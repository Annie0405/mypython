...
def split_train_val_test(root_dir, server_dir, train, val, test, option):
    train, val = map(int, [train, val])
    if test:
        test = int(test)
    else:
        test = 0
    if option == "det":
        split_det(root_dir, server_dir, train, val, test)
    elif option == "rec":
        split_rec(root_dir, server_dir, train, val, test)


def split_det(root_dir, server_dir, train: int, val: int, test: int):
    print("split_det")
